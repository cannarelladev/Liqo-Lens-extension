...
export const offloadNamespace = async (
  nsOffloadingStore: Renderer.K8sApi.KubeObjectStore<NamespaceOffloading>,
  namespace: string,
  nodes: string[] = [],
  ...
  return nsOffloadingStore.create(
    { name: "offloading", namespace: namespace },
    { spec: offloadingSpec }
  );
};
...
export async function peerWithCluster(
  ...
  try {
    await Renderer.K8sApi.secretsApi.create(
      { name: "remote-token-" + clusterID, namespace: "liqo" },
      {
        metadata: {
          labels: {
    ...
    if (e?.error?.reason != "AlreadyExists") throw e;
...
export async function toggleOutgoingPeering(
  fcStore: Renderer.K8sApi.KubeObjectStore<ForeignCluster>,
  clusterID: string,
  enabled: boolean
): Promise<ForeignCluster> {
...
export async function getPeeringParameters() {
  const tokenSecret = await Renderer.K8sApi.secretsApi.get({
    name: "auth-token",
    namespace: "liqo",
  });
  if (!tokenSecret) {
    throw new Error("No auth token secret found");
  }
  const token = atob(tokenSecret.data["token"]);

  const clusterIdentityCm = await Renderer.K8sApi.configMapApi.get({
    name: "liqo-clusterid-configmap",
    namespace: "liqo",
  });
  if (!clusterIdentityCm) {
    throw new Error("No auth token secret found");
  }
  ...
  if (!authNodeList) {
    throw new Error("No auth node found");
  }
  const authNode = authNodeList.find(
    (node) =>
      node.metadata.labels &&
      !(node.metadata.labels["liqo.io/type"] == "virtual-node")
  );
  if (!authNode) {
    throw new Error("No auth node found");
  }
  const authIP = authNode.status.addresses
    ? authNode.status.addresses[0].address
    : "";
  const authSvc = await Renderer.K8sApi.serviceApi.get({
    name: "liqo-auth",
    namespace: "liqo",
  });
  if (!authSvc) {
